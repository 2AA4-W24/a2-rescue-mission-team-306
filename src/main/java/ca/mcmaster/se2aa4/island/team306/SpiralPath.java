...
public class SpiralPath implements Path{
     ...
     */
    public SpiralPath(Coords start, Coords end, Direction heading, GameMap map){
    ...
    private DecisionQueue turnRight(){
        ...
        if(left == SpiralMapValue.OUT_OF_RANGE ){
            queue.enqueue(repositionRight());
        }
        if (left2 == SpiralMapValue.OUT_OF_RANGE){
            queue.enqueue(repositionRight());
        }
        if (up2 == SpiralMapValue.OUT_OF_RANGE){
            queue.enqueue(flatRight());
            return queue;
        }

        queue.enqueue(SpiralMover.deriveFly(facing));

        queue.enqueue(SpiralMover.deriveTurn(facing.getLeft()));
    ...
    private DecisionQueue turnLeft(){
        ...
        MapValue up2 = map.checkCoords(pos.step(facing).step(facing));
        if(right == SpiralMapValue.OUT_OF_RANGE ){
            queue.enqueue(repositionLeft());
        }
        if (right2 == SpiralMapValue.OUT_OF_RANGE){
            queue.enqueue(repositionLeft());
        }
        if (up2 == SpiralMapValue.OUT_OF_RANGE){
        ...
        queue.enqueue(SpiralMover.deriveTurn(facing.getRight()));
    ...
    private DecisionQueue moveForward(){
        DecisionQueue queue = new DecisionQueue();
        queue.enqueue(SpiralMover.deriveFly(heading));
